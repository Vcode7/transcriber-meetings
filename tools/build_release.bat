...
set OLD_INSTALLER=%PROJECT_ROOT%\installer\dist\Setup_AIMeetingTranscriber_v3.2.1.exe
set OLD_APP_DIR=%PROJECT_ROOT%\build\old_v3.2.1_temp
...
REM ── 1. Silently extract old v3.2.1 installer ─────────────────
echo [1/3] Silently installing old v3.2.1 to temp folder...
if not exist "!OLD_INSTALLER!" (
    echo ERROR: Old v3.2.1 installer not found at: !OLD_INSTALLER!
    exit /b 1
)
...
if !ERRORLEVEL! neq 0 (
    echo ERROR: Silent installation of v3.2.1 failed.
    exit /b 1
)
echo [OK] Old v3.2.1 extracted to temporary directory.
echo.
